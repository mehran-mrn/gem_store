...
namespace Mehran\Zarinpal\Helpers;

use Webkul\Sales\Repositories\OrderRepository;
use Webkul\Sales\Repositories\InvoiceRepository;

/**
 * Zarinpal ipn listener helper
 *
 * @author    Jitendra Singh <jitendra@Mehran.com>
 * @copyright 2018 Mehran Software Pvt Ltd (http://www.webkul.com)
 */
class Ipn
{
    /**
    ...
    /**
     * This function process the ipn sent from Zarinpal end
     *
     * @param array $post
     * @return void
     */
    ...
    /**
     * Post back to Zarinpal to check whether this request is a valid one
     *
    ...
    protected function postBack()
    {
        if (array_key_exists('test_ipn', $this->post) && 1 === (int) $this->post['test_ipn'])
            $url = config('payment.drivers.Zarinpal.apiPaymentUrl');
        else
            $url = config('payment.drivers.Zarinpal.apiPaymentUrl');

        // Set up request to Zarinpal
        $request = curl_init();
